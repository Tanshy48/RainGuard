    ...
    inner class weatherTask() : AsyncTask<String, Void, String>() {
        ...
        override fun onPostExecute(result: String?) {
            ...
            try {
                ...
                lange = intent.getStringExtra("lang").toString()
                when (lange){
                    "0" ->  updatedAtText = "Updated at: "+ SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.ENGLISH).format(Date(updatedAt*1000))
                    "1" -> updatedAtText = "Updated at: "+ SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.UK).format(Date(updatedAt*1000))
                    else -> updatedAtText = "Updated at: "+ SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.ENGLISH).format(Date(updatedAt*1000))
                }
                ...
                lange = intent.getStringExtra("lang").toString()
                when (lange){
                    "0" -> { findViewById<TextView>(R.id.sunrise).text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(sunrise*1000))
                        findViewById<TextView>(R.id.sunset).text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(sunset*1000))}
                    "1" -> { findViewById<TextView>(R.id.sunrise).text = SimpleDateFormat("hh:mm a", Locale.UK).format(Date(sunrise*1000))
                        findViewById<TextView>(R.id.sunset).text = SimpleDateFormat("hh:mm a", Locale.UK).format(Date(sunset*1000))}
                    else -> { findViewById<TextView>(R.id.sunrise).text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(sunrise*1000))
                        findViewById<TextView>(R.id.sunset).text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(sunset*1000))
                    }
                }
